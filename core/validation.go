...
// This file defines public workflow validation types and a simple factory/registry
// so internal packages can register full implementations.
...
// simpleWorkflowValidator is a basic no-op validator used as fallback
...
func (wv *simpleWorkflowValidator) ValidateComposition(agents []Agent, mode string) WorkflowValidationResult {
	// Minimal validation: valid if at least one agent
	if len(agents) == 0 {
