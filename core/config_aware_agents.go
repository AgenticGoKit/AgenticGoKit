...
func NewConfigAwareUnifiedAgent(name string, config *ResolvedAgentConfig, capabilities map[CapabilityType]AgentCapability, handler AgentHandler) *ConfigAwareUnifiedAgent {
	unifiedAgent := NewUnifiedAgent(name, capabilities, handler)
...
func (ca *ConfigAwareUnifiedAgent) Run(ctx context.Context, state State) (State, error) {
...
func (ca *ConfigAwareUnifiedAgent) GetLLMConfig() *ResolvedLLMConfig {
	if ca.config != nil {
...
func (ca *ConfigAwareUnifiedAgent) UpdateConfiguration(config *ResolvedAgentConfig) error {
	if config == nil {
	...
	if config.LLMConfig != nil {
		if llmCap, exists := ca.capabilities[CapabilityTypeLLM]; exists {
...
func (ca *ConfigAwareUnifiedAgent) ApplySystemPrompt(ctx context.Context, state State) (State, error) {
...
type ConfigAwareSequentialAgent struct {
	name   string
	agents []ConfigAwareAgent
	config *ResolvedAgentConfig
}

// NewConfigAwareSequentialAgent creates a new configuration-aware sequential agent
func NewConfigAwareSequentialAgent(name string, config *ResolvedAgentConfig, agents ...ConfigAwareAgent) *ConfigAwareSequentialAgent {
	// Filter out disabled agents
	enabledAgents := make([]ConfigAwareAgent, 0, len(agents))
...
func (ca *ConfigAwareSequentialAgent) Run(ctx context.Context, initialState State) (State, error) {
	if !ca.IsEnabled() {
...
func (ca *ConfigAwareSequentialAgent) GetLLMConfig() *ResolvedLLMConfig {
	if ca.config != nil {
		return ca.config.LLMConfig
	}
	return nil
...
func (ca *ConfigAwareSequentialAgent) UpdateConfiguration(config *ResolvedAgentConfig) error {
	if config == nil {
		return fmt.Errorf("configuration cannot be nil")
	}
	ca.config = config
...
func (ca *ConfigAwareSequentialAgent) ApplySystemPrompt(ctx context.Context, state State) (State, error) {
	if ca.config == nil || ca.config.SystemPrompt == "" {
		return state, nil
	}
...
func NewConfigAwareParallelAgent(name string, config *ResolvedAgentConfig, agents ...ConfigAwareAgent) *ConfigAwareParallelAgent {
	// Filter out disabled agents
	enabledAgents := make([]ConfigAwareAgent, 0, len(agents))
	for _, agent := range agents {
		if agent != nil && agent.IsEnabled() {
...
func (ca *ConfigAwareParallelAgent) Run(ctx context.Context, initialState State) (State, error) {
	if !ca.IsEnabled() {
...
func (ca *ConfigAwareParallelAgent) GetLLMConfig() *ResolvedLLMConfig {
	if ca.config != nil {
		return ca.config.LLMConfig
	}
	return nil
...
func (ca *ConfigAwareParallelAgent) UpdateConfiguration(config *ResolvedAgentConfig) error {
	if config == nil {
		return fmt.Errorf("configuration cannot be nil")
	}
	ca.config = config
...
func (ca *ConfigAwareParallelAgent) ApplySystemPrompt(ctx context.Context, state State) (State, error) {
	if ca.config == nil || ca.config.SystemPrompt == "" {
		return state, nil
	}
...
func (ca *ConfigAwareParallelAgent) runParallelExecution(ctx context.Context, initialState State) (State, error) {
