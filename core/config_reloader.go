...
type ConfigReloadCallback func(*Config, error)

// DefaultConfigReloader implements the ConfigReloader interface
...
func NewConfigReloader(validator ConfigValidator, agentManager AgentManager) *DefaultConfigReloader {
...
func (r *DefaultConfigReloader) StopWatching() error {
	...
	Logger().Info().Msg("Stopped watching configuration file")
	return nil
}
...
func (r *DefaultConfigReloader) OnConfigChanged(callback ConfigReloadCallback) {
	r.mutex.Lock()
	defer r.mutex.Unlock()
...
func (r *DefaultConfigReloader) watchLoop() {
	...
	for {
		select {
		case <-r.ctx.Done():
			Logger().Debug().Msg("Config watcher stopping due to context cancellation")
			return

		case event, ok := <-r.watcher.Events:
			if !ok {
				Logger().Debug().Msg("Config watcher events channel closed")
				return
			}
		...
		case err, ok := <-r.watcher.Errors:
			if !ok {
				Logger().Debug().Msg("Config watcher errors channel closed")
				return
			}
...
func (r *DefaultConfigReloader) handleConfigChange() error {
	...
	resolver := NewConfigResolver(newConfig)
	err = resolver.ApplyEnvironmentOverrides()
	if err != nil {
		Logger().Error().
			Err(err).
...
func (r *DefaultConfigReloader) notifyCallbacks(config *Config, err error) {
	r.mutex.RLock()
...
func (r *DefaultConfigReloader) GetCurrentConfig() *Config {
	r.mutex.RLock()
	defer r.mutex.RUnlock()
