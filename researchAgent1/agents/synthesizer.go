...
// SynthesizerHandler represents a custom synthesizer agent handler.
// 
...
func NewSynthesizerFromConfig(config agenticgokit.ResolvedAgentConfig) (*SynthesizerHandler, error) {
...
func (a *SynthesizerHandler) Run(ctx context.Context, event agenticgokit.Event, state agenticgokit.State) (agenticgokit.AgentResult, error) {
	...
	for i := 2 - 1; i >= 0; i-- {
		if i < len(agents) {
			...
			if agentResponse, exists := state.Get(fmt.Sprintf("%s_response", prevAgentName)); exists {
				...
				logger.Debug().Str("agent", "synthesizer").Str("source_agent", prevAgentName).Interface("input", agentResponse).Msg("Processing previous agent's output")
				found = true
				break
			}
		}
	...
	if !found {
		...
		logger.Debug().Str("agent", "synthesizer").Interface("input", inputToProcess).Msg("Processing original message (final fallback)")
	}
	...
	if mcpManager != nil {
		...
		logger.Debug().Str("agent", "synthesizer").Int("tool_count", len(availableTools)).Msg("MCP Tools discovered")
		toolsPrompt = agenticgokit.FormatToolsPromptForLLM(availableTools)
	} else {
		logger.Warn().Str("agent", "synthesizer").Msg("MCP Manager is not available")
	}
	...
	logger.Debug().Str("agent", "synthesizer").Str("system_prompt", systemPrompt).Str("user_prompt", userPrompt).Msg("Full LLM prompt")
	...
	if err != nil {
		...
		return agenticgokit.AgentResult{}, fmt.Errorf("Synthesizer LLM call failed: %w", err)
	}
	
	logger.Debug().Str("agent", "synthesizer").Str("response", response.Content).Msg("Initial LLM response received")
	...
	logger.Debug().Str("agent", "synthesizer").Str("llm_response", response.Content).Msg("LLM response for tool call analysis")
	logger.Debug().Str("agent", "synthesizer").Interface("parsed_tool_calls", toolCalls).Msg("Parsed tool calls from LLM response")
	
	// Execute any requested tools
	if len(toolCalls) > 0 && mcpManager != nil {
		logger.Info().Str("agent", "synthesizer").Int("tool_calls", len(toolCalls)).Msg("Executing LLM-requested tools")
		...
		for _, toolCall := range toolCalls {
			if toolName, ok := toolCall["name"].(string); ok {
				...
				logger.Info().Str("agent", "synthesizer").Str("tool_name", toolName).Interface("args", args).Msg("Executing tool as requested by LLM")
				
				// Execute tool using the global ExecuteMCPTool function
				result, err := agenticgokit.ExecuteMCPTool(ctx, toolName, args)
				if err != nil {
					logger.Error().Str("agent", "synthesizer").Str("tool_name", toolName).Err(err).Msg("Tool execution failed")
					mcpResults = append(mcpResults, fmt.Sprintf("Tool '%s' failed: %v", toolName, err))
				} else {
					if result.Success {
						logger.Debug().Str("agent", "synthesizer").Str("tool_name", toolName).Msg("Tool execution successful")
						...
						logger.Error().Str("agent", "synthesizer").Str("tool_name", toolName).Msg("Tool execution was not successful")
		...
		logger.Debug().Str("agent", "synthesizer").Msg("No tool calls requested or MCP manager not available")
	}
	...
	if len(mcpResults) > 0 {
		...
		if err != nil {
			return agenticgokit.AgentResult{}, fmt.Errorf("Synthesizer final LLM call failed: %w", err)
		}
		finalResponse = finalLLMResponse.Content
		logger.Info().Str("agent", "synthesizer").Str("final_response", finalResponse).Msg("Final response generated with tool results")
	} else {
		finalResponse = response.Content
		logger.Debug().Str("agent", "synthesizer").Msg("Using initial LLM response (no tools used)")
	}
	...
	outputState.Set("synthesizer_response", finalResponse)
	outputState.Set("message", finalResponse)
	
	// TODO: Add custom state data here if needed
	// Examples:
	// outputState.Set("synthesizer_metadata", yourMetadata)
	// outputState.Set("synthesizer_confidence", confidenceScore)
	...
	logger.Debug().Str("agent", "synthesizer").Msg("Agent processing completed successfully")
...
// func (a *SynthesizerHandler) validateInput(input interface{}) error {
...
// func (a *SynthesizerHandler) preprocessInput(input interface{}) (interface{}, error) {
...
// func (a *SynthesizerHandler) postprocessOutput(output string) (string, error) {
...
// func (a *SynthesizerHandler) handleCustomError(err error) error {
...
// func (a *SynthesizerHandler) callExternalAPI(ctx context.Context, data interface{}) (interface{}, error) {
...
// func (a *SynthesizerHandler) saveToDatabase(ctx context.Context, data interface{}) error {
