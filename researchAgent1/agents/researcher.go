...
// ResearcherHandler represents a custom researcher agent handler.
// 
...
func NewResearcherFromConfig(config agenticgokit.ResolvedAgentConfig) (*ResearcherHandler, error) {
...
func (a *ResearcherHandler) Run(ctx context.Context, event agenticgokit.Event, state agenticgokit.State) (agenticgokit.AgentResult, error) {
	...
	// Researcher processes the original input message
	...
	logger.Debug().Str("agent", "researcher").Interface("input", inputToProcess).Msg("Processing original message")
	...
	if mcpManager != nil {
		...
		logger.Debug().Str("agent", "researcher").Int("tool_count", len(availableTools)).Msg("MCP Tools discovered")
		toolsPrompt = agenticgokit.FormatToolsPromptForLLM(availableTools)
	} else {
		logger.Warn().Str("agent", "researcher").Msg("MCP Manager is not available")
	}
	...
	logger.Debug().Str("agent", "researcher").Str("system_prompt", systemPrompt).Str("user_prompt", userPrompt).Msg("Full LLM prompt")
	...
	if err != nil {
		...
		return agenticgokit.AgentResult{}, fmt.Errorf("Researcher LLM call failed: %w", err)
	}
	
	logger.Debug().Str("agent", "researcher").Str("response", response.Content).Msg("Initial LLM response received")
	...
	logger.Debug().Str("agent", "researcher").Str("llm_response", response.Content).Msg("LLM response for tool call analysis")
	logger.Debug().Str("agent", "researcher").Interface("parsed_tool_calls", toolCalls).Msg("Parsed tool calls from LLM response")
	
	// Execute any requested tools
	if len(toolCalls) > 0 && mcpManager != nil {
		logger.Info().Str("agent", "researcher").Int("tool_calls", len(toolCalls)).Msg("Executing LLM-requested tools")
		...
		for _, toolCall := range toolCalls {
			if toolName, ok := toolCall["name"].(string); ok {
				...
				logger.Info().Str("agent", "researcher").Str("tool_name", toolName).Interface("args", args).Msg("Executing tool as requested by LLM")
				
				// Execute tool using the global ExecuteMCPTool function
				result, err := agenticgokit.ExecuteMCPTool(ctx, toolName, args)
				if err != nil {
					logger.Error().Str("agent", "researcher").Str("tool_name", toolName).Err(err).Msg("Tool execution failed")
					mcpResults = append(mcpResults, fmt.Sprintf("Tool '%s' failed: %v", toolName, err))
				} else {
					if result.Success {
						logger.Debug().Str("agent", "researcher").Str("tool_name", toolName).Msg("Tool execution successful")
						...
						logger.Error().Str("agent", "researcher").Str("tool_name", toolName).Msg("Tool execution was not successful")
		...
		logger.Debug().Str("agent", "researcher").Msg("No tool calls requested or MCP manager not available")
	}
	...
	if len(mcpResults) > 0 {
		...
		if err != nil {
			return agenticgokit.AgentResult{}, fmt.Errorf("Researcher final LLM call failed: %w", err)
		}
		finalResponse = finalLLMResponse.Content
		logger.Info().Str("agent", "researcher").Str("final_response", finalResponse).Msg("Final response generated with tool results")
	} else {
		finalResponse = response.Content
		logger.Debug().Str("agent", "researcher").Msg("Using initial LLM response (no tools used)")
	}
	...
	outputState.Set("researcher_response", finalResponse)
	outputState.Set("message", finalResponse)
	
	// TODO: Add custom state data here if needed
	// Examples:
	// outputState.Set("researcher_metadata", yourMetadata)
	// outputState.Set("researcher_confidence", confidenceScore)
	// outputState.Set("processing_time", processingTime)
	
	
	
	
	// Route to the next agent (Analyzer) in the workflow
	outputState.SetMeta(agenticgokit.RouteMetadataKey, "analyzer")
	...
	logger.Debug().Str("agent", "researcher").Msg("Agent processing completed successfully")
...
// func (a *ResearcherHandler) validateInput(input interface{}) error {
...
// func (a *ResearcherHandler) preprocessInput(input interface{}) (interface{}, error) {
...
// func (a *ResearcherHandler) postprocessOutput(output string) (string, error) {
...
// func (a *ResearcherHandler) handleCustomError(err error) error {
...
// func (a *ResearcherHandler) callExternalAPI(ctx context.Context, data interface{}) (interface{}, error) {
...
// func (a *ResearcherHandler) saveToDatabase(ctx context.Context, data interface{}) error {
