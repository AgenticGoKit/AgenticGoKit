...
// AnalyzerHandler represents a custom analyzer agent handler.
// 
...
func NewAnalyzerFromConfig(config agenticgokit.ResolvedAgentConfig) (*AnalyzerHandler, error) {
...
func (a *AnalyzerHandler) Run(ctx context.Context, event agenticgokit.Event, state agenticgokit.State) (agenticgokit.AgentResult, error) {
	...
	found := false
	agents := []string{"researcher", "analyzer", "synthesizer"}
	...
	for i := 1 - 1; i >= 0; i-- {
		if i < len(agents) {
			...
			if agentResponse, exists := state.Get(fmt.Sprintf("%s_response", prevAgentName)); exists {
				...
				logger.Debug().Str("agent", "analyzer").Str("source_agent", prevAgentName).Interface("input", agentResponse).Msg("Processing previous agent's output")
				found = true
				break
			}
		}
	...
	if !found {
		...
		logger.Debug().Str("agent", "analyzer").Interface("input", inputToProcess).Msg("Processing original message (final fallback)")
	}
	...
	if mcpManager != nil {
		...
		logger.Debug().Str("agent", "analyzer").Int("tool_count", len(availableTools)).Msg("MCP Tools discovered")
		toolsPrompt = agenticgokit.FormatToolsPromptForLLM(availableTools)
	} else {
		logger.Warn().Str("agent", "analyzer").Msg("MCP Manager is not available")
	}
	...
	logger.Debug().Str("agent", "analyzer").Str("system_prompt", systemPrompt).Str("user_prompt", userPrompt).Msg("Full LLM prompt")
	...
	if err != nil {
		...
		return agenticgokit.AgentResult{}, fmt.Errorf("Analyzer LLM call failed: %w", err)
	}
	
	logger.Debug().Str("agent", "analyzer").Str("response", response.Content).Msg("Initial LLM response received")
	...
	logger.Debug().Str("agent", "analyzer").Str("llm_response", response.Content).Msg("LLM response for tool call analysis")
	logger.Debug().Str("agent", "analyzer").Interface("parsed_tool_calls", toolCalls).Msg("Parsed tool calls from LLM response")
	
	// Execute any requested tools
	if len(toolCalls) > 0 && mcpManager != nil {
		logger.Info().Str("agent", "analyzer").Int("tool_calls", len(toolCalls)).Msg("Executing LLM-requested tools")
		...
		for _, toolCall := range toolCalls {
			if toolName, ok := toolCall["name"].(string); ok {
				...
				logger.Info().Str("agent", "analyzer").Str("tool_name", toolName).Interface("args", args).Msg("Executing tool as requested by LLM")
				
				// Execute tool using the global ExecuteMCPTool function
				result, err := agenticgokit.ExecuteMCPTool(ctx, toolName, args)
				if err != nil {
					logger.Error().Str("agent", "analyzer").Str("tool_name", toolName).Err(err).Msg("Tool execution failed")
					mcpResults = append(mcpResults, fmt.Sprintf("Tool '%s' failed: %v", toolName, err))
				} else {
					if result.Success {
						logger.Debug().Str("agent", "analyzer").Str("tool_name", toolName).Msg("Tool execution successful")
						...
						logger.Error().Str("agent", "analyzer").Str("tool_name", toolName).Msg("Tool execution was not successful")
		...
		logger.Debug().Str("agent", "analyzer").Msg("No tool calls requested or MCP manager not available")
	}
	...
	if len(mcpResults) > 0 {
		...
		if err != nil {
			return agenticgokit.AgentResult{}, fmt.Errorf("Analyzer final LLM call failed: %w", err)
		}
		finalResponse = finalLLMResponse.Content
		logger.Info().Str("agent", "analyzer").Str("final_response", finalResponse).Msg("Final response generated with tool results")
	} else {
		finalResponse = response.Content
		logger.Debug().Str("agent", "analyzer").Msg("Using initial LLM response (no tools used)")
	}
	...
	outputState.Set("analyzer_response", finalResponse)
	outputState.Set("message", finalResponse)
	
	// TODO: Add custom state data here if needed
	// Examples:
	// outputState.Set("analyzer_metadata", yourMetadata)
	// outputState.Set("analyzer_confidence", confidenceScore)
	// outputState.Set("processing_time", processingTime)
	
	
	
	
	// Route to the next agent (Synthesizer) in the workflow
	outputState.SetMeta(agenticgokit.RouteMetadataKey, "synthesizer")
	...
	logger.Debug().Str("agent", "analyzer").Msg("Agent processing completed successfully")
...
// func (a *AnalyzerHandler) validateInput(input interface{}) error {
...
// func (a *AnalyzerHandler) preprocessInput(input interface{}) (interface{}, error) {
...
// func (a *AnalyzerHandler) postprocessOutput(output string) (string, error) {
...
// func (a *AnalyzerHandler) handleCustomError(err error) error {
...
// func (a *AnalyzerHandler) callExternalAPI(ctx context.Context, data interface{}) (interface{}, error) {
...
// func (a *AnalyzerHandler) saveToDatabase(ctx context.Context, data interface{}) error {
