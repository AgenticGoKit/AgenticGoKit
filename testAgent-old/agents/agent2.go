...
// Agent2Handler represents a custom agent2 agent handler.
// 
...
func NewAgent2FromConfig(config agenticgokit.ResolvedAgentConfig) (*Agent2Handler, error) {
...
func (a *Agent2Handler) Run(ctx context.Context, event agenticgokit.Event, state agenticgokit.State) (agenticgokit.AgentResult, error) {
		...
		for _, toolCall := range toolCalls {
			if toolName, ok := toolCall["name"].(string); ok {
				...
				if err != nil {
					...
					if result.Success {
						logger.Debug().Str("agent", "agent2").Str("tool_name", toolName).Msg("Tool execution successful")
	...
	logger.Debug().Str("agent", "agent2").Msg("Agent processing completed successfully")
