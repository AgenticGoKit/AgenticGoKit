...
func (s *Server) HandleGetAgents(w http.ResponseWriter, r *http.Request) {
	...
	if len(agents) == 0 {
		agents = []map[string]any{{"id": "agent1", "name": "Agent1", "description": "Default agent 1"}, {"id": "agent2", "name": "Agent2", "description": "Default agent 2"}}
	}
...
func (s *Server) HandleChat(w http.ResponseWriter, r *http.Request) {
	...
	if req.UseOrchestration {
		...
		s.Results.Reset()
		tracing.RecordEdge("webui-session", "User", req.Agent, req.Message)
...
func (s *Server) HandleTraceDiagram(w http.ResponseWriter, r *http.Request) {
	...
	diagram, labels := tracing.BuildTraceData(s.Runner, session, linear)
...
func (s *Server) HandleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		case "chat":
			...
			go func(m wsInbound) {
				tracing.RecordEdge("webui-session", "User", m.Agent, m.Message)
