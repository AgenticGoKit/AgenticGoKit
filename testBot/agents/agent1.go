...
// Agent1Handler represents a custom agent1 agent handler.
// 
...
func NewAgent1FromConfig(config agenticgokit.ResolvedAgentConfig) (*Agent1Handler, error) {
...
func (a *Agent1Handler) Run(ctx context.Context, event agenticgokit.Event, state agenticgokit.State) (agenticgokit.AgentResult, error) {
	...
	// Agent1 processes the original input message
	...
	logger.Debug().Str("agent", "agent1").Interface("input", inputToProcess).Msg("Processing original message")
	...
	logger.Debug().Str("agent", "agent1").Str("system_prompt", systemPrompt).Str("user_prompt", userPrompt).Msg("Full LLM prompt")
	...
	if err != nil {
		...
		return agenticgokit.AgentResult{}, fmt.Errorf("Agent1 LLM call failed: %w", err)
	}
	
	logger.Debug().Str("agent", "agent1").Str("response", response.Content).Msg("Initial LLM response received")
	...
	logger.Debug().Str("agent", "agent1").Str("llm_response", response.Content).Msg("LLM response for tool call analysis")
	logger.Debug().Str("agent", "agent1").Interface("parsed_tool_calls", toolCalls).Msg("Parsed tool calls from LLM response")
	...
	if len(toolCalls) > 0 && mcpManager != nil {
		...
		for _, toolCall := range toolCalls {
			if toolName, ok := toolCall["name"].(string); ok {
				...
				logger.Info().Str("agent", "agent1").Str("tool_name", toolName).Interface("args", args).Msg("Executing tool as requested by LLM")
				...
				if err != nil {
					...
					if result.Success {
						logger.Debug().Str("agent", "agent1").Str("tool_name", toolName).Msg("Tool execution successful")
						...
						logger.Error().Str("agent", "agent1").Str("tool_name", toolName).Msg("Tool execution was not successful")
		...
		logger.Debug().Str("agent", "agent1").Msg("No tool calls requested or MCP manager not available")
	}
	...
	if len(mcpResults) > 0 {
		...
		if err != nil {
			return agenticgokit.AgentResult{}, fmt.Errorf("Agent1 final LLM call failed: %w", err)
		}
		finalResponse = finalLLMResponse.Content
		logger.Info().Str("agent", "agent1").Str("final_response", finalResponse).Msg("Final response generated with tool results")
	} else {
		finalResponse = response.Content
		logger.Debug().Str("agent", "agent1").Msg("Using initial LLM response (no tools used)")
	}
	...
	// Examples:
	// outputState.Set("agent1_metadata", yourMetadata)
	// outputState.Set("agent1_confidence", confidenceScore)
	...
	outputState.SetMeta(agenticgokit.RouteMetadataKey, "agent2")
	...
	logger.Debug().Str("agent", "agent1").Msg("Agent processing completed successfully")
...
// func (a *Agent1Handler) validateInput(input interface{}) error {
...
// func (a *Agent1Handler) preprocessInput(input interface{}) (interface{}, error) {
...
// func (a *Agent1Handler) postprocessOutput(output string) (string, error) {
...
// func (a *Agent1Handler) handleCustomError(err error) error {
...
// func (a *Agent1Handler) saveToDatabase(ctx context.Context, data interface{}) error {
