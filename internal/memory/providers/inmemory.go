...
type InMemoryProvider struct {
	...
	messages  map[string][]core.Message // sessionID -> messages
	sessionID string
	...
	documents map[string]core.Document  // documentID -> document metadata
}
...
func (m *InMemoryProvider) Query(ctx context.Context, query string, limit ...int) ([]core.Result, error) {
	m.mutex.RLock()
...
func (m *InMemoryProvider) GetHistory(ctx context.Context, limit ...int) ([]core.Message, error) {
	m.mutex.RLock()
	defer m.mutex.RUnlock()
...
func (m *InMemoryProvider) IngestDocument(ctx context.Context, doc core.Document) error {
	m.mutex.Lock()
	defer m.mutex.Unlock()
...
func (m *InMemoryProvider) IngestDocuments(ctx context.Context, docs []core.Document) error {
...
func (m *InMemoryProvider) SearchKnowledge(ctx context.Context, query string, options ...core.SearchOption) ([]core.KnowledgeResult, error) {
	m.mutex.RLock()
	defer m.mutex.RUnlock()
	...
	for docID, entry := range m.knowledge {
		...
		// Apply filters
		if len(config.Sources) > 0 && !contains(config.Sources, doc.Source) {
			continue
		}
		...
		if len(config.Tags) > 0 && !containsAnyTag(doc.Tags, config.Tags) {
			continue
		}
...
func (m *InMemoryProvider) SearchAll(ctx context.Context, query string, options ...core.SearchOption) (*core.HybridResult, error) {
	start := time.Now()
...
func (m *InMemoryProvider) BuildContext(ctx context.Context, query string, options ...core.ContextOption) (*core.RAGContext, error) {
...
func (m *InMemoryProvider) formatContextText(query string, results *core.HybridResult, history []core.Message, config *core.ContextConfig) string {
