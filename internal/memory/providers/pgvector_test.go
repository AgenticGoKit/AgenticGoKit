...
import (
	...
	// Import internal memory to trigger factory registration
	_ "github.com/kunalkushwaha/agenticgokit/internal/memory"
)
...
func (suite *PgVectorIntegrationTestSuite) SetupSuite() {
	...
	suite.config = core.AgentMemoryConfig{
		Provider:                "pgvector",
		Connection:              connStr,
		MaxResults:              10,
		Dimensions:              1536,
	...
	// Create provider using core factory (which will delegate to internal)
	provider, err := core.NewMemory(suite.config)
...
func BenchmarkPgVectorStore(b *testing.B) {
	...
	provider, err := core.NewMemory(config)
