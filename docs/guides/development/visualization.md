...
// Generate diagram
// Diagrams are generated by the CLI. Use: agentcli visualize or --visualize during scaffold.
fmt.Println(diagram)
```
...
// Use CLI flags to customize output location and style.
```
...
if builder.CanVisualize() {
    // Note: runtime builders do not generate diagrams; use CLI visualization.
    // Save to file
...
// See: agentcli create ... --visualize or agentcli visualize
```
...
// Example orchestration (config-driven)
// Build with core.NewRunnerFromConfig and visualize with CLI.
    WithAgents(serviceHandlers).
    WithTimeout(1 * time.Minute).
    WithMaxConcurrency(20)

// Generate diagram via CLI, not at runtime.

// API routing
// For routing/mixed modes, prefer configuration and CLI diagrams.
    WithAgents(serviceHandlers).
    WithTimeout(30 * time.Second)

// Use CLI to generate routing diagrams.
```
...
if builder.CanVisualize() {
    // Diagrams are produced by CLI tools, not at runtime.
...
### Composition Builder
// Diagram generation is provided by the CLI. There is no runtime API.
