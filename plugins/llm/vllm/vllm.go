// Package vllm provides a vLLM LLM provider plugin for AgenticGoKit.
package vllm
...
// factory creates a vLLM provider from configuration
func factory(cfg core.LLMProviderConfig) (core.ModelProvider, error) {
	// Build vLLM config from core config
	vllmConfig := llm.VLLMConfig{
		BaseURL:           cfg.BaseURL,
		APIKey:            cfg.APIKey,
		Model:             cfg.Model,
		...
		TopK:              cfg.VLLMTopK,
		TopP:              float32(cfg.VLLMTopP),
		MinP:              float32(cfg.VLLMMinP),
		PresencePenalty:   float32(cfg.VLLMPresencePenalty),
		FrequencyPenalty:  float32(cfg.VLLMFrequencyPenalty),
		RepetitionPenalty: float32(cfg.VLLMRepetitionPenalty),
		BestOf:            cfg.VLLMBestOf,
		UseBeamSearch:     cfg.VLLMUseBeamSearch,
		LengthPenalty:     float32(cfg.VLLMLengthPenalty),
		StopTokenIds:      cfg.VLLMStopTokenIds,
		SkipSpecialTokens: cfg.VLLMSkipSpecialTokens,
		IgnoreEOS:         cfg.VLLMIgnoreEOS,
		Stop:              cfg.VLLMStop,
		HTTPTimeout:       cfg.HTTPTimeout,
	}

	wrapper, err := llm.NewVLLMAdapterWrapped(vllmConfig)
	if err != nil {
		return nil, err
	}
...
func init() {
	core.RegisterModelProviderFactory("vllm", factory)
}
